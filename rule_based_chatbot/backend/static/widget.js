(function() {
  ...
  const styles = `
    ...
    .suggestion-btn:hover { background: #f8fafc; transform: translateY(-1px); }
  `;
  ...
  window.initFYJCBot = function(userConfig = {}) {
    ...
    function addSuggestions(suggestions) {
      ...
      suggestions.forEach((item) => {
        ...
        button.onclick = async () => {
          ...
          try {
            const res = await fetch(`${config.apiHost}/api/chat/select`, {
              method: 'POST',
            ...
            if (data.type === 'answer' && data.answer) {
              ...
              addMessage('Sorry, could not fetch the suggested answer.', 'bot-msg');
            }
          } catch (err) {
            if (messages.contains(typingDiv)) messages.removeChild(typingDiv);
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (words.length < 2) {
          addMessage("Please enter at least 2 words for a more accurate response. / अधिक अचूक प्रतिसादासाठी कृपया किमान २ शब्द टाका.", 'bot-msg');
          input.value = '';
          return;
      }
      ...
      try {
        const res = await fetch(`${config.apiHost}/api/chat/ask`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (data.type === 'answer' && data.answer) {
          ...
          addMessage('No answer found. Please try rephrasing your question.', 'bot-msg');
        }
      } catch (err) {
        if (messages.contains(typingDiv)) messages.removeChild(typingDiv);
        addMessage('Something went wrong. Please try again.', 'bot-msg');
